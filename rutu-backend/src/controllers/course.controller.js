...
const createCourse = catchAsync(async (req, res) => {
  ...
  // Trigger Notifikasi (Asumsi req.body.tutorId tersedia)
  if (req.body.tutorId) {
    await notificationService.createNotification(
      req.body.tutorId,
      "Kursus Dibuat",
      `Kursus "${req.body.name}" berhasil dipublikasikan.`,
      "COURSE_CREATED",
    );
  }
...
const getCourseById = catchAsync(async (req, res) => {
  ...
  res
    .status(200)
    .json({ success: true, message: "Detail kursus ditemukan", data: course });
});
